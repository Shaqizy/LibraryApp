...
class PublisherController(val publisherService: PublisherService) {
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Удаление издательства по ее идентификатору")
    fun deletePublisher(@PathVariable id: String): ResponseEntity<InfoMessageDto>{
        publisherService.deleteById(id)
        return ResponseEntity(InfoMessageDto("Deleted Publisher with id = $id successfully!"), HttpStatus.OK)
    }
}